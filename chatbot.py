...
class Chatbot:
    def __init__(self, size_layer, num_layers, embedded_size,
        ...
        def cells(reuse=False):
            return tf.keras.layers.LSTMCell(size_layer, recurrent_initializer=tf.orthogonal_initializer())
        ...
        with tf.compat.v1.variable_scope("encoder_embeddings"):        
            
            encoder_embeddings = tf.Variable(tf.random.uniform([from_dict_size, embedded_size], -1, 1))
            encoder_embedded = tf.nn.embedding_lookup(encoder_embeddings, self.X)
            main = tf.strided_slice(self.X, [0, 0], [batch_size, -1], [1, 1])
            
        with tf.compat.v1.variable_scope("decoder_embeddings"):        
            decoder_input = tf.concat([tf.fill([batch_size, 1], GO), main], 1)
            decoder_embeddings = tf.Variable(tf.random.uniform([to_dict_size, embedded_size], -1, 1))
            decoder_embedded = tf.nn.embedding_lookup(encoder_embeddings, decoder_input)
        
        with tf.compat.v1.variable_scope("encoder"):
            rnn_cells = tf.keras.layers.StackedRNNCells([cells() for _ in range(num_layers)])
            _, last_state = keras.layers.RNN(rnn_cells, encoder_embedded)
